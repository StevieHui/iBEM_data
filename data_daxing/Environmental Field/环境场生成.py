...
### 画图 ###
fig = plt.figure(figsize=(10, 5.4))  # 设置画布大小

# 图1绘制温度场
#im = plt.pcolormesh(y1, x1, yflat_T, shading='auto', cmap='rainbow', vmin=22.5, vmax=23.5)
im = plt.pcolormesh(y1, x1, yflat_T, shading='auto', cmap='rainbow', vmin=19.5, vmax=22.5)
...
#cd = plt.colorbar(im, ticks=np.linspace(22.5, 23.5, 5), orientation='vertical', pad=0.05)  # pad控制间距
cd = plt.colorbar(im, ticks=np.linspace(19.5, 22.5, 7), orientation='vertical', pad=0.05)  # pad控制间距
...
### 画图 ###
fig2 = plt.figure(figsize=(10, 5.4))  # 设置画布大小
...
### 画图 ###
fig3 = plt.figure(figsize=(10, 5.4))  # 设置画布大小

# 图3绘制CO2场
# im3 = plt.pcolormesh(y1, x1, yflat_C, shading='auto', cmap='rainbow', vmin=550, vmax=750)
im3 = plt.pcolormesh(y1, x1, yflat_C, shading='auto', cmap='rainbow', vmin=600, vmax=800)
...
# cd3 = plt.colorbar(im3, ticks=np.linspace(550, 750, 5), orientation='vertical')  # pad控制间距
cd3 = plt.colorbar(im3, ticks=np.linspace(600, 800, 5), orientation='vertical')  # pad控制间距
...
### 画图 ###
fig4 = plt.figure(figsize=(10, 5.4))  # 设置画布大小
...
im4 = plt.pcolormesh(y1, x1, yflat_PM, shading='auto', cmap='rainbow', vmin= 0, vmax= 35)
...
cd4 = plt.colorbar(im4, ticks=np.linspace(0, 35, 7), orientation='vertical')  # pad控制间距
...
plt.axis('off')  # 关闭坐标轴
plt.tight_layout()  # 调整布局以避免重叠
plt.savefig('./pic/pm2.5/PM25_' + date + '_' + lap + '.png', dpi=300, bbox_inches='tight')  # 保存为PNG格式，分辨率300dpi，自动调整边界
#plt.show()
...
### 画图 ###
fig5 = plt.figure(figsize=(10, 5.4))  # 设置画布大小
...
### 画图 ###
fig6 = plt.figure(figsize=(10, 5.4))  # 设置画布大小

# 图5绘制湿度场
# im6 = plt.pcolormesh(y1, x1, yflat_H, shading='auto', cmap='rainbow', vmin=25, vmax=50)
im6 = plt.pcolormesh(y1, x1, yflat_H, shading='auto', cmap='rainbow', vmin=30, vmax=40)
im6_points = plt.plot(y, x, "x", c='black', label="input point")  # 标出测点位置
# cd6 = plt.colorbar(im6, ticks=np.linspace(45, 65, 5), orientation='vertical')  # pad控制间距
# cd6 = plt.colorbar(im6, ticks=np.linspace(25, 50, 6), orientation='vertical')  # pad控制间距
cd6 = plt.colorbar(im6, ticks=np.linspace(30, 40, 6), orientation='vertical')  # pad控制间距
cd6.set_label('Humidity (%)', size=15, rotation=90, fontname='Times New Roman')  # 色阶标签竖直显示
cd6.ax.yaxis.set_major_formatter(FormatStrFormatter('%d'))
cd6.ax.tick_params(labelsize=12)  # 设置刻度字号
for t in cd6.ax.get_yticklabels():  # 遍历 colorbar 刻度标签
    t.set_fontname('Times New Roman')

plt.axis('off')  # 关闭坐标轴
plt.tight_layout()  # 调整布局以避免重叠
plt.savefig('./pic/humi/Humi_' + date + '_' + lap + '.png', dpi=300, bbox_inches='tight')  # 保存为PNG格式，分辨率300dpi，自动调整边界
#plt.show()
